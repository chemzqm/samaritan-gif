...
function drawFirstFrame(ctx, frame, opt) {
  ctx.font = opt.fontSize + 'px MagdaCleanMono'
  var space = ''
  var text = uid(opt.first.length).split("").join(space)
  ...
  ctx.stroke()
  // triangle
  drawTriangle(ctx, frame.index * 0.2, opt)
}
...
function drawText(ctx, frame, opt) {
  if (frame.end) return
  ctx.font = opt.fontSize + 'px MagdaCleanMono'
  var text = frame.text.split("").join('')
...
function drawFrame(config, frame, cb) {
  ...
  if (frame.begin === true) {
    drawFirstFrame(ctx, frame, config)
  } else if (frame.end) {
...
var draw = module.exports = function (config, cb) {
  ...
  frames = frames.concat(createStartFrames(texts[0]))
  config.first = texts[0]
  for (var i = 0; i < total; i++) {
  ...
  tmp.dir({mode: 0750, prefix: uid(8)}, function (err, dir) {
    ...
    frames.forEach(function (frame, i) {
      frame.opacity = Math.max(0, 1 - ((i - 5 + 10)%10)/7)
      frame.index = i
      p.add(function (done) {
    ...
    p.done(function (err) {
      if (err) return cb(err)
      exec('convert -delay 8 -loop 0 *.png animates.gif', {cwd: dir}, function (err) {
        if (err) return cb(err)
