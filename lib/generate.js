...
function drawStartText(ctx, frame, opt) {
  ...
  if (frame.percent) {
    ...
    ctx.font = Math.floor(size) + 'px Fira Mono'
    arr.forEach(function (o) {
...
var draw = module.exports = function (config, cb) {
  ...
  tmp.dir({mode: 0750, prefix: uid(8)}, function (err, dir) {
    ...
    frames.forEach(function (frame, i) {
      var d = i%10
      frame.opacity = 1 *(Math.abs(d - 5)/5)
      frame.index = i
      p.add(function (done) {
    ...
    p.done(function (err) {
      if (err) return cb(err)
      exec('convert -delay 10 -loop 0 *.png animates.gif', {cwd: dir}, function (err) {
        if (err) return cb(err)
